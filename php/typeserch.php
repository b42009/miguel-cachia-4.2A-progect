...
if(!isset($type) || $type == null){header('Location:http://localhost/fpage.php');}
error_reporting(0);
    session_start();

    $conn = mysqli_connect('localhost', 'root', '', 'events') or die('Cannot connect to db');
    $query = "select eventId,name, eventDate,imagLink from event WHERE imgcategory='1' and eventDate >= CURDATE() and type =(select typeId from eventtypes where typeName = '$type')";
